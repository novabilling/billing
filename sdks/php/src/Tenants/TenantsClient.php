...
namespace NovaBilling\Tenants;

use Psr\Http\Client\ClientInterface;
use NovaBilling\Core\Client\RawClient;
use NovaBilling\Types\TenantResponse;
...
use NovaBilling\Tenants\Requests\UpdateTenantDto;
use NovaBilling\Types\TenantUsageResponse;
use NovaBilling\Tenants\Requests\TestSmtpTenantsRequest;
...
class TenantsClient
{
     ...
     */
    public function getMe(?array $options = null): TenantResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return TenantResponse::fromJson($json);
            }
     ...
     * @throws NovabillingApiException
     */
    public function updateMe(UpdateTenantDto $request = new UpdateTenantDto(), ?array $options = null): TenantResponse
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
     ...
     * @throws NovabillingApiException
     */
    public function getUsage(?array $options = null): TenantUsageResponse
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                $json = $response->getBody()->getContents();
                return TenantUsageResponse::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new NovabillingException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
     ...
     *
     * @param TestSmtpTenantsRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * @throws NovabillingApiException
     */
    public function testSmtp(TestSmtpTenantsRequest $request, ?array $options = null): MessageResponse
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
