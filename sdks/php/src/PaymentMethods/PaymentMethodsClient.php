...
namespace NovaBilling\PaymentMethods;
...
use NovaBilling\PaymentMethods\Requests\CreatePaymentMethodDto;
use NovaBilling\Types\PaymentMethodResponse;
use JsonException;
...
class PaymentMethodsClient
{
    ...
    /**
     * @param CreatePaymentMethodDto $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * @throws NovabillingApiException
     */
    public function create(CreatePaymentMethodDto $request, ?array $options = null): PaymentMethodResponse
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
     ...
     * @throws NovabillingApiException
     */
    public function getByCustomer(string $customerId, ?array $options = null): array
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                $json = $response->getBody()->getContents();
                return JsonDecoder::decodeArray($json, [PaymentMethodResponse::class]); // @phpstan-ignore-line
            }
        } catch (JsonException $e) {
            throw new NovabillingException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
     ...
     * @throws NovabillingApiException
     */
    public function get(string $id, ?array $options = null): PaymentMethodResponse
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
     ...
     * @throws NovabillingApiException
     */
    public function setDefault(string $id, ?array $options = null): PaymentMethodResponse
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
