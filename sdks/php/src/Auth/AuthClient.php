...
namespace NovaBilling\Auth;

use Psr\Http\Client\ClientInterface;
use NovaBilling\Core\Client\RawClient;
use NovaBilling\Auth\Requests\RegisterDto;
use NovaBilling\Types\RegisterResponse;
...
use NovaBilling\Auth\Requests\LoginDto;
use NovaBilling\Types\LoginResponse;
use NovaBilling\Auth\Requests\RefreshTokenDto;
use NovaBilling\Types\TokenPairResponse;
use NovaBilling\Auth\Requests\ForgotPasswordDto;
use NovaBilling\Types\MessageResponse;
use NovaBilling\Auth\Requests\ResetPasswordDto;
...
class AuthClient
{
     ...
     */
    public function register(RegisterDto $request, ?array $options = null): RegisterResponse
    {
     ...
     * @throws NovabillingApiException
     */
    public function login(LoginDto $request, ?array $options = null): LoginResponse
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
     ...
     * @throws NovabillingApiException
     */
    public function refreshToken(RefreshTokenDto $request, ?array $options = null): TokenPairResponse
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                $json = $response->getBody()->getContents();
                return TokenPairResponse::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new NovabillingException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
     ...
     *
     * @param ForgotPasswordDto $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * @throws NovabillingApiException
     */
    public function forgotPassword(ForgotPasswordDto $request, ?array $options = null): MessageResponse
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
     ...
     * @throws NovabillingApiException
     */
    public function resetPassword(ResetPasswordDto $request, ?array $options = null): MessageResponse
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
